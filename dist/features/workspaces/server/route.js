...
const app = new Hono()
    ...
    const { workspaceId } = ctx.req.param();
    const { name, image } = ctx.req.valid('form');
    const existingWorkspace = await databases.getDocument(DATABASE_ID, WORKSPACES_ID, workspaceId);
    const member = await getMember({
        databases,
        workspaceId,
        userId: user.$id,
    ...
    const tasks = await databases.listDocuments(DATABASE_ID, TASKS_ID, [Query.equal('workspaceId', workspaceId)]);
    // delete members
    ...
    if (existingWorkspace.imageId)
        storage.deleteFile(IMAGES_BUCKET_ID, existingWorkspace.imageId);
