...
const app = new Hono()
    ...
    const projects = await databases.listDocuments(DATABASE_ID, PROJECTS_ID, projectIds.length > 0 ? [Query.contains('$id', projectIds)] : []);
    ...
    const updatedTasks = await Promise.all(tasks.map(async (task) => {
        ...
        return databases.updateDocument(DATABASE_ID, TASKS_ID, $id, { status, position });
    }));
